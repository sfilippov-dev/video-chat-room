 ...
 *
 * Формулировки безглагольные, и это не стилистика, а необходимость: имя
 * участника — произвольная строка, рода которой приложение не знает. «Вера
 * присоединился к комнате» ошибается на каждом втором имени, а угадывать род
 * по окончанию — гадание, которое ломается на Никите, Саше и любом имени не на
 * русском. Состояние («в комнате» / «больше не в комнате») верно для любого
 * имени и заодно точнее описывает то, что знает сервер.
 *
 * Про выход говорим одинаково и на кнопку «Выйти», и на обрыв связи: сервер
 * эти случаи не различает, поэтому обещать «соединение потеряно» он не вправе.
 */
function systemText({ name, text }) {
  if (text === SYSTEM_EVENTS.JOINED) return `${name} в комнате`;
  if (text === SYSTEM_EVENTS.LEFT) return `${name} больше не в комнате`;
  return name;
}
