...
**Формулировка системных сообщений.** В истории хранится семантический код (`'joined'` / `'left'`), текст собирается на клиенте: «<Имя> в комнате» / «<Имя> больше не в комнате». Формулировки безглагольные намеренно: имя участника — произвольная строка, рода которой приложение не знает, и «Вера присоединился к комнате» ошибается на каждом втором имени. Определять род по окончанию — гадание, которое ломается на Никите, Саше и любом имени не на русском. Формулировка «соединение потеряно» **не используется нигде** — PRD п. 31 её прямо запрещает, потому что сервер не может достоверно отличить обрыв связи от закрытия вкладки.
...
Оба пути ведут в один и тот же серверный код: сервер **не различает** осознанный выход и обрыв — и поэтому системное сообщение всегда «больше не в комнате» (PRD п.31). Автопереподключения нет: вернуться можно только повторным входом по ссылке (Non-Goals).
...
| `disconnect` рассылает `peer-left` и системное сообщение с формулировкой «больше не в комнате» | п.31 |
...
12. Закрытие вкладки → плитка исчезает, системное сообщение «<Имя> больше не в комнате».
